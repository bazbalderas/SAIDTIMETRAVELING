...
function updateNavigation() {
    ...
    Object.keys(screens).forEach(key => {
        ...
        btn.className = `flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold transition-all duration-200 ${
            isActive ? 'bg-blue-600 text-white shadow-lg shadow-blue-200/50' : 'text-gray-700 hover:bg-gray-100'
        }`;
        ...
        mobileBtn.className = `flex-shrink-0 px-3 py-1 rounded-lg text-xs font-medium ${
...
function ActionCard(iconName, title, description, action, primary = false) {
    return `
        <div class="border border-gray-200 rounded-xl p-6 bg-white hover:shadow-lg transition-all duration-300 flex flex-col justify-between">
            ...
            <button class="w-full px-4 py-2 text-sm font-medium rounded-lg transition-colors ${
                primary ? 'bg-blue-600 hover:bg-blue-700 text-white shadow-md' : 'bg-gray-100 hover:bg-gray-200 text-gray-700'
            }" onclick="${action}">
    ...
    content.innerHTML = `
        <div class="space-y-8">
            ...
            <div class="flex flex-wrap gap-3 border-b border-gray-200 pb-3">
                ${tabs.map((tab, i) => `
                    <button onclick="cambiarTabDataEntry('${tab}')" class="px-4 py-2 rounded-lg font-medium transition-colors text-sm ${
                        appState.dataEntryTab === tab ? 'bg-blue-600 text-white shadow-md' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-4 py-3">
                                    <span class="inline-flex px-3 py-1 text-xs font-semibold rounded-full ${
                                        horas > 0 ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'
